...
async function handleClick(row, col) {
                ...
                if (output === "PROMOTE") {
                    if (movedPiece instanceof Pawn) {
                        showPromotionMenu(choice => {
                            ...
                            saveGameState(board, null, turn, 'kingLevel');
    ...
    redraw();
    await saveGameState(board, clickedPiece, turn, 'kingLevel');
...
async function restartGame() {
    ...
    redraw();
    gameFinished = false;
    await saveGameState(board, null, 'white', 'kingLevel');
}
...
async function blackBot() {
    await saveGameState(board, null, 'black', 'kingLevel');
...
export function startGame() {
    ...
    document.getElementById("levelMessage").textContent =
        ...
        Captured pieces spawn at the originally occupied square    
        and automatically become Evolved.  
        Can capture ally pieces up to 3 times.
